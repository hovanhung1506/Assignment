window.addEventListener('load', function () {
  ...
  const courseItems = document.querySelectorAll('.courses .list__courses .course__item');
  const scheduleItems = document.querySelectorAll('.schedule .list__courses .course__item');
  ...
  function activeTabCourse(navs, c) {
    [...navs].forEach((course) => {
      course.addEventListener('click', function () {
        ...
        const tabIndex = course.dataset.index;
        const main = course.parentNode.nextElementSibling;
        const wrappers = main.querySelectorAll(`.${c}-wrapper`);
  ...
  document.body.addEventListener('click', function (e) {
    // console.log(!!e.target.closest('.icon__search-m'));
    if (
  ...
  activeTabCourse(courseItems, 'course');
  activeTabCourse(scheduleItems, 'schedule');
  toggleSearch();
  backToTop();
});
